...
def _status_offline(pc_id: str, message: str) -> PcStatusProbeResult:
    insert_log(action="status", pc_id=pc_id, status="offline", message=message)
...
def _probe_ping(ip_value: ipaddress.IPv4Address, pc_id: str) -> PcStatusProbeResult:
    ...
    try:
        ...
        insert_log(action="status", pc_id=pc_id, status="failed", message=message)
    ...
    if result.returncode == 0:
        ...
        insert_log(action="status", pc_id=pc_id, status="online", message=message)
