...
- `backend/.env` に `APP_VERSION` / `APP_BUILD` を設定すると、`/api/health` とヘッダー表示の版情報を任意値で上書きできる（未設定時は `APP_VERSION=dev`、`APP_BUILD=git短縮SHA`。`git` 解決不可時は `APP_BUILD=local` にフォールバック）。
